...
export default function Breadcrumb() {
    ...
    return (
        <div className="flex items-center gap-[8px] m-[32px_40px]">
            ...
            {pathname.split("/").slice(1).map(path => (
                <div key={path} className="flex items-center gap-[8px]">
                    <Image alt="breadcrumb chevron" src={chevronRight} width={24} height={24} />
                    <Link className="flex gap-1 capitalize text-[14px] leading-[169%] -tracking-[1%] font-light" href={path}>{path.split("-").map(segment => <span key={segment}>{segment}</span>)}</Link>
                </div>
            ))}
        </div>
    )
